...
//Clase que implementa el controlador GenerarDomiciliosGaraje.
//Crea un conjunto de domicilios de tipo Garaje.
public class GenerarDomiciliosGaraje extends ViewBaseAction implements IForwardAction {
	...
	public void execute() throws Exception {
		...
		Integer ginicio = direccion.getGarajeIni().intValue();
		Integer gfinal = direccion.getGarajeFin().intValue();
		...
		if (direccion.getIdDireccion() == null){
			...
			return;
				...
				while (ldomi.hasNext()) {
					...
					if (rdomicilio.getNumGaraje() == null){
						continue;
					}
					if ((rdomicilio.getNumGaraje() >= direccion.getGarajeIni()) && (rdomicilio.getNumGaraje() <= direccion.getGarajeFin())) 
						numeroDomicilios++;
					}
				if (numeroDomicilios > 0){
					msg = "Ya existen Domicilios Garaje en este rango.";
					addError(msg);
					return;
					}
			}
		...
		for (int i = direccion.getGarajeIni(); i <= direccion.getGarajeFin(); i++) {
			...
			String dirCompleta1 = dirCompleta + " Garaje " + i;
		...
		msg = "Se han creado " + cuentaDomicilios + " Domicilios Garaje en esta Dirección.";
		this.addMessage(msg);
		}
